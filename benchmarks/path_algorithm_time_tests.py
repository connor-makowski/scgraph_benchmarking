...
from benchmarks.utils.graphs import make_nxgraph, igraph_from_scgraph, make_gridgraph, get_nx_shortest_path, get_igraph_shortest_path
...
for name, scgraph_object in graph_data:
    ...
    if 'gridgraph' not in name.lower():
        ...
        print(f"  Skipping CH/TNR preprocessing ({graph_nodes:,} not a grid graph).")
        ch_build_ms = None
        tnr_build_ms = None
    ...
    for case_name, origin, destination in test_cases:
        ...
        try:
            ...
            print(f"Skipping due to error solving with SCGraph Dijkstra: {e}")
            continue
